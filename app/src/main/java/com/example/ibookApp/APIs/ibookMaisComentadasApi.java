...
public class ibookMaisComentadasApi {
    private static final String API_ENDPOINT = BASE_URL_API + "Obras/ObrasMaisComentadasSemana";

    public interface ibookMaisListListener {
        void onBookListReceived(List<obrasDTO> bookList);
    }

    public static void getBookList(ibookMaisListListener listener) {
        new ibookMaisListAsyncTask(listener).execute();
    }

    private static class ibookMaisListAsyncTask extends AsyncTask<Void, Void, List<obrasDTO>> {
        private String query;
        private ibookMaisListListener listener;

        public ibookMaisListAsyncTask(ibookMaisListListener listener) {
            this.query = query;
